...
hold on;
hnewvehi1=plot(newvehi(1,:)+x,newvehi(2,:)+y,trajectory(1,:),trajectory(2,:),'r:');hold off
...
theta=theta+25*pi/180;%‚í‚´‚Æ‚¸‚ç‚µ‚Ä‚Ý‚éB
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	if(~isempty(linesensind))
		subplot(2,2,4);
		plot(linesensind([1,1],:),[1,1;0,0],'o-',mean(linesensind),0,'rp');
		axis([-10,10,0,1.2]);
	end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	if(isempty(linesensind)) break;end
	steering=-mean(linesensind)*10;
	theta = theta+speed*dt/D*tan(steering*pi/180);
	theta1=theta+pi/2;
	x = x + speed*dt*cos(theta1);
	y = y + speed*dt*sin(theta1);
	trajectory=[trajectory,[x;y]];
	set(hnewvehi1(2),'xdata',trajectory(1,:),'ydata',trajectory(2,:));
	drawnow;%pause(0.5);
end
